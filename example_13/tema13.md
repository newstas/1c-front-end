...
`moveTo(x, y)` — перемещает перо в координаты, указанные с помощью xи y.
Удобно использовать для размещения начальной точки в любом месте.

`lineTo(x, y)` — рисует линию от текущей позиции рисования до позиции, указанной с помощью xи y.
