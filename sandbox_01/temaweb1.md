...
с помощью которой можно вычислить поддельные письма. DKIM добавляет в письмо цифровую 
подпись. Благодаря ей почтовые провайдеры (Mail.ru, Gmail) могут проверить, что сообщение 
отправлено именно с указанного домена.
...
Имя записи | Тип записи | Значение
---        | ---        | ---
site.ru    | TXT        | `v=DKIM1; k=rsa; p=XXXXXXXXXXXXXXXXXXXXX,`

- где v — версия DKIM, всегда принимает значение v=DKIM1;
...
например [NsLookup](https://www.nslookup.io/) ). 
...
Здесь у нас есть три тега: `v`, `p` и `rua` , которые имеют значения `DMARC1`, `ignore` и 
