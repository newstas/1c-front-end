...
**Корневая папка** (корневая **директория** / корневой **каталог** / **корень** документа) — это 
основная папка, **в которой хранятся все каталоги и файлы сайта**. Именно в эту папку 
загружается архив с файлами сайта и базы данных.
...
[Что такое рекурсия, рекурсивный и итеративный процесс в программировании][ref_016]
...
> В среднем для DNS-поиска потребуется от 1 до 4 запросов, но время этого поиска будет составлять всего несколько миллисекунд.
...
- [Ассоциации операторов Корневых серверов DNS][ref_015]
...
- **NSD** (Name Server Daemon) — сервер DNS с открытым исходным кодом (не реализует рекурсивные запросы и кэширование)
...
[Онлайн инструмент Dig][ref_002] — проверка записей DNS от Google:

Онлайн инструмент Dig – это веб-аналог UNIX-команды dig | Набора [инструментов администратора Google][ref_003]
...
**dig, host, nslookup** — утилиты командной строки для обращения к системе DNS.
  ...
  Является аналогом утилиты nslookup и host 
  - **host** — утилита в UNIX-системах (DNS-клиент; аналог в Windows — nslookup)
  - **nslookup** (англ. name server lookup поиск на сервере имён) — Утилита портирована на Windows непосредственно фирмой Microsoft и поставляется вместе с операционной системой.
...
[Что такое DNS? Введение в систему доменных имён][ref_018]

[DNS - SkillFactory][ref_017]
...
[ref_015]:<https://root-servers.org/>
[ref_016]:<https://ru.hexlet.io/blog/posts/recursive>
[ref_017]:<https://blog.skillfactory.ru/glossary/dns/>
[ref_018]:<(https://tproger.ru/explain/domain-name-system)>
