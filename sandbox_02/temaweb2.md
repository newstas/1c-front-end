...
потому что когда-то браузеры другие не поддерживали.
...
### Значение `inline` (`display: inline;`)
...
### Значение `inline-block` (`display: inline-block;`)
...
### Значения `flex` (`display: flex;`) создает `flexbox` - *из темы 3*
