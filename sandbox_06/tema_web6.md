...
На этом занятии: познакомимся:

- познакомимся с основами **стурук данных** в JS
  - научимся пользоваться **словарем типа Map**
  - научимся пользоваться **массивом (Array)**

- изучим **свойства и методы** массива (Array):
  - `length`
  - `pop()`
  - `push()`
  - `splice()`
...
- **Комбинирование структур** - создадим объект, представляющий утку, содержащий различные типы данных
- **Игра "Викторина"** - с вопросами и ответами, реализуемые на языке программирования JS
...
```js
let array = new Array(); // создание массива с помощью конструктора Array()
let array = []; // литеральная запись массива
```
...
Массив – это особый подвид объектов. Квадратные скобки, используемые для того, чтобы получить доступ к свойству `array[0]` – это по сути обычный синтаксис доступа по ключу, как `obj[key]`, где в роли `obj` у нас `array`, а **в качестве ключа – числовой индекс**.
...
## Использование массива (свойство `length`)

- `length` - свойство, которое служит для определения колличества элементов в массиве


## Изменение массива (методы)

- `array.splice(index, 1)` - удаляет элемент с номером index;
- `array.splice(index, 0, value)` - сдвинет элементы, начиная с номера index, вправо и на освободившееся место вставит объект value;
- `array.pop()` - удалит последней элемент;
- `array.push(value)` - добавит объект value в конец.

Необходимо догадаться, каким будет массив `colors` в конце:
...
- `classList.add()` - метод добавляет элементу класс, указанный в скобках.

- `classList` - свойство интерфейса `Element`

Интерфейс `Element` представляет собой один из объектов в `Document`. Этот интерфейс описывает методы и свойства, общие для всех видов элементов.


#### Свойства `classList`

- `length` - свойество объекта `classList`, которое возвращает количество классов у элемента
...
- `add()` - метод объекта `classList` позволяет добавлять CSS классы элементу
...
```js
let obj = Object.create({x:1, y:2}); // o1 наследует свойства x и y
```
...
Это называется «простым объектом» («plain object») или просто `Object`.
...
У них есть свои особенности. Иногда люди говорят что-то вроде «тип данных Array» или «тип данных Date», но формально они не являются отдельными типами, а относятся к типу данных `Object`. Они лишь расширяют его различными способами.

**Объекты можно использовать** как **ассоциативные массивы для хранения пар ключ/значение**.

## Наследование и цепочка прототипов

Модель наследования в JavaScript - динамическая и не включает в себя реализацию понятия `class` (хотя ключевое слово `class` в ES6 представляет собой лишь "синтаксический сахар" поверх **прототипно-ориентированной модели наследования**)
...
**Set** имеет очень полезное **свойство** под названием `size`, которое 

- `size` - дает информацию о количестве значений внутри коллекции
