...
## 7.5.1.1 Создание элементов

Полный код в файле: *index_creat_elem_7511.html*
...
## 7.5.1.2 Обратный вызов

Полный код в файле: *index_callback_7512.html*

Внутри обратного вызова доступ к оъекту, который является источником события,
осуществляется при помощи ключевого слова `this`:

```js
$(".block").on("click", function () { // подключаем обработчик события к созданному новому элементу с классом .block
    $(this).toggleClass("block--active"); // добавляет или удаляет заданный классы по принципу переключателя между .block и block--active
});
```
