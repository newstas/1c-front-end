...
- 3.2.1 Поворот - `rotate`

Значение `rotate` *пример:* `transform: rotate(45deg);`
(*поворот на 45 градусов по часовой стрелке*)

- 3.2.2 Масштабирование - `scale`

Значение `scale` *пример:* `transform: scaleX(1.5) scaleY(0.5);
(*увеличить по горизонтали в 1,5 раза и уменьшить по вертикали в 2 раза*)

- 3.2.3 Перекос - `skew` 

Значение `skew` *пример:* `transform: skew(30deg, 0deg);`
